...
def compute_accuracy_Iter1(ref_model, evalloader, start_class, end_class):
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    #tg_feature_model.eval()
    correct1 = 0
    total = 0
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(evalloader):
            inputs, targets = inputs.to(device), targets.to(device)
            total += targets.size(0)
            outputs = ref_model(inputs, cls_fc=True)
            # for i in range(args.num_cls):
            outputs_1 = outputs[:, :20]
...
class TEST:
    ...
    def compute_accuracy_test(self, ref_model, evalloader, start_class, end_class):
        device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        ...
        for batch_idx, (inputs, targets) in enumerate(evalloader):
            ...
            for i in range(int(self.epochs/self.val_epoch)):
                ...
                exit()
                _, predicted1 = outputs_eval.max(1)

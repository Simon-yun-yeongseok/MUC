...
##### seperate train_data & test_data(after download) ####
...
for n_run in range(args.nb_runs):
    ...
    for iteration in range(start_iter, int(args.num_classes/args.nb_cl)):
        ...
        indices_train_subset = np.array([i in order[range(iteration*args.nb_cl,(iteration+1)*args.nb_cl)] for i in Y_train_total]) ### need translation
        indices_test_subset  = np.array([i in order[range(0,(iteration+1)*args.nb_cl)] for i in Y_valid_total])                    ### need translation
